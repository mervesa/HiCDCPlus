...
generate.binned.gi.list <- function(binsize, chrs = NULL, Dthreshold = 2e+06, gen = "Hsapiens", gen_ver = "hg19") {
    ...
    if (is.null(chrs)) {
        ...
        chrs <- chrs[chrs %in% get.chrs(gen, gen_ver)]
    }
    for (chrom in chrs) {
        seqlen <- get.chr.sizes(gen, gen_ver)[chrom]
