...
#'See \code{?gi.list.validate} and \code{gi.list.binsize.detect} for details.
...
#'@examples gi_list<-generate.binned.gi.list(50e3,chrs='chr22')
#'gi_list<-add.hic.counts(gi_list,
#'hic_path=system.file("extdata", "GSE63525_HMEC_combined_example.hic",
#' package = "HiCDCPlus"))
#'@export

add.hic.counts <- function(gi_list, hic_path, chrs = NULL, add_inter = FALSE) {
    ...
    for (chrom in chrs) {
        ...
        count_matrix <- tryCatch(
            ...
            tryCatch(straw(norm = "NONE", fn = path.expand(hic_path), bs = binsize, ch1 = chrom, ch2 = chrom, 
                      u = "BP"),
                     error=function(e){
                      straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select,ch2=chr_select,u="BP")   
                     })
            })
        }else{
            count_matrix<-straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select,ch2=chr_select,u="BP")   
        }
        count_matrix <- count_matrix %>% dplyr::filter(abs(.data$y - .data$x) <= Dthreshold) %>% dplyr::rename(startI = "x", 
            startJ = "y")
        gi_list[[chrom]] <- add.2D.features(gi_list[[chrom]], count_matrix)
    ...
    if (add_inter) {
                ...
                count_matrix <- tryCatch(
                    ...
                    error=function(e){
                        tryCatch(straw(norm = "NONE", fn = path.expand(hic_path), bs = binsize, ch1 = chrom, ch2 = chr2, 
                              u = "BP"),
                              error=function(e){
                                  straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select,ch2=chr_other,u="BP")   
                              })
                    })
                }else{
                    count_matrix<-straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select,ch2=chr_other,u="BP")   
        ...
        gi_list <- add.1D.features(gi_list, inter_matrix, chrs, agg = sum)
