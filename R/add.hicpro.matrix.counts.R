...
add.hicpro.matrix.counts <- function(gi_list, absfile_path, matrixfile_path, chrs = NULL, add_inter = FALSE) {
    ...
    for (chrom in chrs) {
        ...
        gi_list[[chrom]] <- add.2D.features(gi_list[[chrom]], count_matrix_chr)
    ...
    if (add_inter) {
        ...
        rm(absI, absJ)
        gi_list <- add.1D.features(gi_list, count_matrix, chrs, agg = sum)
        rm(count_matrix)
    }
    return(gi_list)
}
