...
#'written using \code{hicdc2hic} or \code{hic2icenorm.gi.list}), 'KR' for
...
#'@examples eigenvector_filepaths<-extract.hic.eigenvectors(
#'hicfile=system.file("extdata", "GSE63525_HMEC_combined_example.hic",
#'package = "HiCDCPlus"),
#'chrs=c("chr22"),binsize=50e3)
#'@export

extract.hic.eigenvectors<-function(hicfile,mode='KR',binsize=100e3,
                                   chrs=NULL,gen="Hsapiens",gen_ver="hg19"){
    options(scipen=9999)
    filepaths<-c()
    if (is.null(chrs)){chrs<-get.chrs(gen,gen_ver)}
