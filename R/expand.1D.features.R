...
#'\code{?add.1D.features} and \code{expand.1D.features}) in sequence is
#'recommended instead of using \code{add.2D.features} directly
#'for each chromosome.
#'@import BSgenome
#'@importFrom dplyr %>%
...
expand.1D.features <- function(gi_list, chrs = NULL, features = NULL, agg = transform.vec) {
    ...
    # arguments default function transform.vec from HiCDC (Carty et al., 2017) For efficient use of memory, add/expand 1D
    # features where applicable is suggested instead of add.2D.features for each chromosome
