...
hic2icenorm.gi.list<-function(hic_path,binsize=50000,chrs=NULL,hic_output=FALSE,
                              gen="Hsapiens",gen_ver="hg19",Dthreshold=Inf){
#convert .hic files to HiTC object with ICE normalized counts
if (is.null(chrs)){
  chrs<-get.chrs(gen,gen_ver)[chrs%in%c("chrY","chrM")]
}
...
count_matrix <- tryCatch(
  ...
  error=function(e){
    tryCatch(straw(norm = "NONE", fn = path.expand(hic_path), bs = binsize, ch1 = chrom1, ch2 = chrom2, 
                   u = "BP"),
             error=function(e){
               straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select1,ch2=chr_select2,u="BP")   
             })
  })
}else{
    count_matrix<-straw.dump(norm = "NONE",fn=path.expand(hic_path),bs=binsize,ch1=chr_select1,ch2=chr_select2,u="BP")   
}
xgi<-generate.binned.gi.list(binsize,chrs=chrom1,gen=gen,gen_ver=gen_ver)[[chrom1]]@regions
ygi<-generate.binned.gi.list(binsize,chrs=chrom2,gen=gen,gen_ver=gen_ver)[[chrom2]]@regions
...
gi_list<-HTClist2gi.list(htc_list,Dthreshold=Dthreshold)
rm(htc_list)
if(hic_output){
